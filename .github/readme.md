...
---
# Note: If you wish to assist with the project, the most of the code is quite stable, so we don't need developers; all we need is someone to summit lists url(s).
---
...
Why are exclusion domains not retrieved from the url?
- The issue with obtaining it from a url is that if you want to add your own local exclusion domain, it must be part of the official repository.
