...
func findTheDomains(url string, saveLocation string) {
	...
	for _, content := range returnContent {
		...
		if len(foundDomain) < 255 && checkIPAddress(foundDomain) && strings.Contains(foundDomain, ".") && !strings.Contains(foundDomain, "#") && !strings.Contains(foundDomain, "*") && !strings.Contains(foundDomain, "!") {
